...
gene_count_D30_df_filtered <- readRDS(here("bulkRNAseq", "results", "gene_count_D30_df_filtered_removeS24.rds"))
sample_metadata_df <- read_tsv(here("bulkRNAseq", "results", "sample_metadata_D30_df_removeS24.txt"))
...
write_tsv(dge_exh.prog.vs.term,
          here("bulkRNAseq", "results", "dge_D30_exh.prog.vs.term_removeS24.txt"))
...
counts_mtx_list[["progenitor"]] <- gene_count_D30_df_filtered %>%
  ...
  column_to_rownames("gene_name") %>%
  as.matrix()
counts_mtx_list[["transitory"]] <- gene_count_D30_df_filtered %>%
...
write_tsv(results_list$progenitor,
          here("bulkRNAseq", "results", "dge_D30_progexh_genotype.Enhdel.v.WT_removeS24.txt"))
write_tsv(results_list$transitory,
          here("bulkRNAseq", "results", "dge_D30_transexh_genotype.Enhdel.v.WT_removeS24.txt"))
write_tsv(results_list$terminal,
          here("bulkRNAseq", "results", "dge_D30_termexh_genotype.Enhdel.v.WT_removeS24.txt"))
